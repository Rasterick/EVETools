...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* Function to use the data generated by the PHP Script */
  async function fetchAndRenderSystem(systemIdToLoad) {
    ...
    if (svgElement) svgElement.innerHTML = ""; // Clear SVG
    ...
    try {
      ...
      if (data && data.length > 0) {
        currentSystemCelestials = data; // Store the fetched data

        // Update the H1 title with the system name (assuming first item is star with system name)
        // You might need to adjust how you get the system name if it's not in celestialData
        let systemDisplayName = `System ${systemIdToLoad}`;
        if (
          currentSystemCelestials[0] &&
          currentSystemCelestials[0].itemName.includes(" - Star")
        ) {
          systemDisplayName = currentSystemCelestials[0].itemName
            .split(" - Star")[0]
            .trim();
        }
        //const appTitle = document.querySelector("#appHeader h1");
        const appTitle = document.getElementById("currentSystemTitleHeader"); // NEW, using the ID
        if (appTitle)
          appTitle.textContent = `System Information - ${systemDisplayName}`;
        if (appTitle)
          appTitle.textContent = `System Information - ${systemDisplayName}`;
        if (document.title)
          document.title = `EVE System Mapper - ${systemDisplayName}`;

        // Now call your existing render function, ensuring it uses currentSystemCelestials
        if (typeof renderSystemSVG === "function") {
          renderSystemSVG(currentSystemCelestials); // Pass the fetched data to renderSystemSVG
        } else {
          console.error("renderSystemSVG is not defined!");
        }
      } else {
        console.error(
          "No data received or data is empty for system ID:",
          systemIdToLoad
        );
        alert(`No celestial data found for system ID: ${systemIdToLoad}`);
      }
    } catch (error) {
