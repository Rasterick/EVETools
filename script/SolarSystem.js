...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchAndRenderSystem(systemIdentifier) {
    ...
    if (selectRefsModal && selectRefsModal.style)
      selectRefsModal.style.display = "none"; // New Modal UI
    ...
    if (signatureZonesGroup && signatureZonesGroup.style)
      signatureZonesGroup.style.display = "none";
    else if (signatureZonesGroup) signatureZonesGroup.innerHTML = "";
    ...
    console.log(
      "All previous system state and UI cleared/reset for new system load."
    );
    ...
    try {
      ...
      if (data && data.systemInfo) {
        ...
        if (appTitleH1)
          appTitleH1.textContent = `System Information - ${
            si.systemName || systemIdentifier
          }`;
        document.title = `EVE System Mapper - ${
          si.systemName || systemIdentifier
        }`;
        ...
        if (sysStatic1El)
          sysStatic1El.textContent = `${si.static1_type || "N/A"}${
            si.static1_leadsTo ? " -> " + si.static1_leadsTo : ""
          }`;
        if (sysStatic2El)
          sysStatic2El.textContent = `${si.static2_type || "N/A"}${
            si.static2_leadsTo ? " -> " + si.static2_leadsTo : ""
          }`;
      } else {
        ...
        const appTitleH1 = document.getElementById("currentSystemTitleHeader");
        if (appTitleH1)
          appTitleH1.textContent = `System Information - ${systemIdentifier} (Info N/A)`;
        if (hdrSysClassEl) hdrSysClassEl.textContent = "N/A";
        if (hdrSysEffectEl) hdrSysEffectEl.textContent = "N/A";
        if (hdrSysStaticsEl) hdrSysStaticsEl.textContent = "N/A";
        if (sysClassEl) sysClassEl.textContent = "N/A";
        if (sysEffectEl) sysEffectEl.textContent = "N/A";
        if (sysStatic1El) sysStatic1El.textContent = "N/A";
        if (sysStatic2El) sysStatic2El.textContent = "N/A";
      }
      ...
      if (currentSystemCelestials.length > 0) {
        ...
        console.error("No celestial data for ID:", systemIdentifier);
        alert(`No celestial data for ${systemIdentifier}.`);
      }
    } catch (error) {
      console.error("Error fetching/processing system data:", error);
