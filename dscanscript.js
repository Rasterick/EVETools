...
// *** NEW BUTTON ELEMENT ***
const saveAndShareButtonElem = document.getElementById("saveAndShareButton");
...
const unrecognizedEntriesTableContainerElem = document.getElementById(
...
async function loadScanDataFromServer(scanId) {
  try {
    const response = await fetch(
      `load_scan.php?id=${encodeURIComponent(scanId)}`
    );
    if (!response.ok) {
    ...
    if (serverData.success && serverData.data_payload) {
      ...
      throw new Error(
        serverData.message || "Failed to retrieve scan data from server."
      );
    }
  } catch (e) {
    console.error("Error loading scan data from server:", e);
    alert(
      "Error loading shared scan: " + e.message + "\nDisplaying a blank slate."
    );
...
function populateFieldsFromDataObject(dataObject) {
  ...
  if (dataObject.playersInStructures !== undefined) {
    playersInStructuresInputElem.value = dataObject.playersInStructures;
  }
    ...
    if (shipsData.length > 0) {
      ...
      if (initSystemName && jPatternInit.test(initSystemName)) {
        ...
        initTitle =
          ...
          escapeHtml(systemNameUpper) +
          "</a> (Ship data loading...)";
      } else if (initSystemName && !jPatternInit.test(initSystemName)) {
        initAdditionalHtml =
...
function performAnalysis() {
  ...
  function getSystemDisplayInfo(baseTitle) {
    ...
    if (systemNameInputValue && jSystemPattern.test(systemNameInputValue)) {
  ...
  const currentUrlParamsForHash = new URLSearchParams(window.location.search);
  ...
  lines.forEach((line) => {
    ...
    if (parts.length < 3) {
      ...
      if (idShipResult.foundShip) {
        if (!firstShipDataForEffectiveName[idShipResult.effectiveName]) {
        ...
        unrecognizedEntries[idShipResult.effectiveName] =
          (unrecognizedEntries[idShipResult.effectiveName] || 0) + 1;
        if (!firstShipDataForEffectiveName[idShipResult.effectiveName]) {
          firstShipDataForEffectiveName[idShipResult.effectiveName] = null;
        }
      }
    }
  });
  ...
  const sortedUnrecognizedNames = Object.keys(unrecognizedEntries).sort(
...
function displayCurrentTimeDate(
  ...
  if (
    (playersInStructures && String(playersInStructures).trim() !== "") ||
...
async function handleSaveAndShare() {
  ...
  try {
    ...
    if (result.success && result.url) {
      navigator.clipboard
        ...
        .catch((err) => {
          ...
          saveAndShareButtonElem.textContent = "Link Ready (Copy Failed)";
          alert(
      ...
      throw new Error(result.message || "Failed to save scan data on server.");
    }
    // ...
  } catch (error) {
    ...
    if (
      ...
      displayMessage +=
        "\nServer DB Error: " + error.serverErrorDetails.deforebug_pdo_exception;
    }
...
saveAndShareButtonElem.addEventListener("click", handleSaveAndShare);
